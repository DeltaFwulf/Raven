...
def physicsIntegration():
    ...
    fig = mlab.figure()
    mesh = mlab.triangular_mesh(pts[:,0], pts[:,1], pts[:,2], tris, figure=fig)
    #omegaArrow = mlab.quiver3d(comFrame.translation[0], comFrame.translation[1], comFrame.translation[2], omega[0,0], omega[0,1], omega[0,2], color=(1.0, 0.0, 1.0), scale_factor=norm(omega[0,:]), figure=fig)
    ...
    def anim(plays:int=1):
        ...
        while(n < plays):
            ...
            for i in range(0, t.size):
                ...
                #omegaArrow.mlab_source.trait_set(u=omega[i,0], v=omega[i,1], w=omega[i,2])
